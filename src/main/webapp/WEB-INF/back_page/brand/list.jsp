...
<head>
    ...
    <script>
        function myclick(pageNo) {
            var name = $("#queryName").val();
            var aa = $("#queryDisplay").val();
            var path = '/brand/list.do?pageNo=' + pageNo + '&name=' + name + '&isDisplay=' + aa;
        ...
        function optDelete() {
            $("#coreForm").attr("action", "/brand/deletes.do")
            $("#coreForm").submit();
        }
    </script>
</head>
...
<div class="body-box">
    <form action="/brand/list.do" method="post" style="padding-top:5px;">
        品牌名称: <input type="text" id="queryName" name="name" value="${brand.name }"/>
        <select name="isDisplay" id="queryDisplay">
            <option value="1"
                    <c:if test="${brand.isDisplay == 1 }">selected="selected"</c:if> >是
            </option>
            <option value="0" <c:if test="${brand.isDisplay == 0 }">selected="selected"</c:if>>不是</option>
        </select>
        ...
        <table cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
            ...
            <c:forEach items="${pageInfo.list }" var="entry">
                ...
                <tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
                    ...
                    <td align="center">
                        <a class="pn-opt" href="/brand/toEdit.do?id=${entry.id}">修改</a> | <a class="pn-opt"
    ...
    <div class="page pb15">
        ...
        <%--<a onclick="myclick(${page})" href="#">${page}</a>--%>
        <%--</c:forEach>--%>
        <%--</span>--%>
        <nav aria-label="Page navigation">
            <ul class="pagination pull-right">
                <li>
                    <a href="#" onclick="myclick(${pageInfo.navigateFirstPage})" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                    </a>
                </li>
                <c:forEach items="${pageInfo.navigatepageNums }" var="page">
                    <li><a onclick="myclick(${page})" href="#">${page}</a></li>
                </c:forEach>
                <li>
                    <a href="#" onclick="myclick(${pageInfo.navigateLastPage})" aria-label="Next">
